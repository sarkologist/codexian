export function updateContextRowHasContent(contextRowEl: HTMLElement): void {
  ...
  const chatIndicator = contextRowEl.querySelector('.claudian-chat-selection-indicator');
  ...
  const hasChatSelection = !!chatIndicator && !chatIndicator.hasClass('claudian-hidden');
  ...
  contextRowEl.classList.toggle(
    'has-content',
    hasEditorSelection || hasBrowserSelection || hasChatSelection || hasCanvasSelection || hasFileChips || hasImageChips
  );
}
